...
class MicroserviceModule(val environment: Environment, val configuration: Configuration) extends AbstractModule with ServicesConfig {
  ...
  def configure(): Unit = {
    ...
    bindProperty1Param("auth.stride.enrolment")
  ...
  private def bindProperty1Param(propertyName: String) =
    bind(classOf[String]).annotatedWith(Names.named(propertyName)).toProvider(new PropertyProviderFor1Param(propertyName))
